...
//                self.layer.shadowOffset = CGSizeMake(0,0);
    ...
    [backView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(ws).with.offset(LENGTH(57));
        make.top.equalTo(ws).with.offset(LENGTH(0));
        make.right.equalTo(ws).with.offset(-LENGTH(57));
        make.bottom.equalTo(ws).with.offset(-LENGTH(0));
        make.height.mas_equalTo(self->backView.mas_width).multipliedBy(0.306);
    }];
    
    title = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(51,51,51) LabelFont:TextFont(20) TextAlignment:NSTextAlignmentLeft Text:@""];
    title.numberOfLines = 3;
    [self addSubview:title];
    [title mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(self->backView.mas_left).with.offset(LENGTH(53));
        make.right.mas_equalTo(self->backView.mas_right).with.offset(-LENGTH(53));
        make.top.mas_equalTo(LENGTH(31));
