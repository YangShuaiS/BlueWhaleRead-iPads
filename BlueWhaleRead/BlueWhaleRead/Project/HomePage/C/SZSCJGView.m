...
- (void)addView{
    ...
    szl = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(255,255,255) LabelFont:TextFont(30) TextAlignment:NSTextAlignmentCenter Text:@"小朋友你本次的识字量为："];
    [backView addSubview:szl];
    
    [szl mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo(self->backView.mas_centerX);
        make.top.mas_equalTo(self->topImageView.mas_bottom).with.offset(50);
    }];
    
    zishu= [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(255,255,255) LabelFont:TextFont(40) TextAlignment:NSTextAlignmentCenter Text:@"200字"];
    [backView addSubview:zishu];
    
    [zishu mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo(self->backView.mas_centerX);
        make.top.mas_equalTo(self->szl.mas_bottom).with.offset(30);
    }];
    
    zql= [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:ChangYongTitLe LabelFont:TextFont(30) TextAlignment:NSTextAlignmentCenter Text:@"小天预估你的认字量是        字\n多读书就可以认识更多的字哦，加油！"];
    zql.numberOfLines = 0;
    [self addSubview:zql];
    
    [zql mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo(self->backView.mas_centerX);
        make.top.mas_equalTo(self->backView.mas_bottom).with.offset(56);
    }];
    ...
    [RJOrFanHuiBookCity mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self->zql.mas_bottom).with.offset(LENGTH(90));
        make.centerX.mas_equalTo(self->backView.mas_centerX);
