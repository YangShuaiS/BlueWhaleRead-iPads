...
- (void)addView{
    ...
    userImageView.layer.cornerRadius = LENGTH(33)/2;
    [self addSubview:userImageView];
    [userImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(ws).with.offset(LENGTH(29));
        make.top.mas_equalTo(ws).with.offset(LENGTH(28));
        make.bottom.mas_equalTo(ws).with.offset(-LENGTH(28));
        make.width.mas_equalTo(LENGTH(33));
        make.height.mas_equalTo(LENGTH(33));
    }];
    
    xz = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:ChangYongTitLe LabelFont:TextFont(20) TextAlignment:NSTextAlignmentLeft Text:@"张小祁刚刚认领了“勤奋之星”勋章"];
    [self addSubview:xz];
    ...
    toptime = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:ChangYongTitLe LabelFont:TextFont(15) TextAlignment:NSTextAlignmentLeft Text:@"2018-05-04"];
    [self addSubview:toptime];
    [toptime mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(ws).with.offset(LENGTH(25));
        make.right.mas_equalTo(ws).with.offset(-LENGTH(25));
    }];

    downtime = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:ChangYongTitLe LabelFont:TextFont(15) TextAlignment:NSTextAlignmentRight Text:@"07:00"];
