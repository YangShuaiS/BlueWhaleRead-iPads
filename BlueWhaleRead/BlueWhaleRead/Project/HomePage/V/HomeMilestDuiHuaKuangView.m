...
- (void)addView{
    ...
    [xinxing mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.mas_equalTo(LENGTH(self->xinxing.image.size.width));
        make.height.mas_equalTo(LENGTH(self->xinxing.image.size.height));
    }];
#pragma legacy ------------------------ 富文本居中？
    title = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(1,117,223) LabelFont:TextFont(20) TextAlignment:NSTextAlignmentCenter Text:@"小朋友你已经读完了 0  本书"];
    [xinxing addSubview:title];
    [title mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self->xinxing.mas_top).with.offset(LENGTH(30));
        make.left.mas_equalTo(self->xinxing);
        make.right.mas_equalTo(self->xinxing);
    }];
    
    subtitle = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(1,117,223) LabelFont:TextFont(20) TextAlignment:NSTextAlignmentCenter Text:@"完成了第 0 个里程碑"];
