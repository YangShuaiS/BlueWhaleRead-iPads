    ...
    for (int i = 0; i < viewarray.count; i++) {
        ...
        if (!lastview) {
            ...
            [view mas_makeConstraints:^(MASConstraintMaker *make) {
                make.bottom.equalTo(ws.mas_bottom).with.offset(-LENGTH(154));
                make.centerX.mas_equalTo(ws.mas_centerX);
            }];
            [tiankong mas_makeConstraints:^(MASConstraintMaker *make) {
                make.left.mas_equalTo(view.mas_right).with.offset(-LENGTH(40));
                make.bottom.mas_equalTo(view.mas_top).with.offset(LENGTH(40));
            }];
        }else{
            [view mas_makeConstraints:^(MASConstraintMaker *make) {
                if (i == 1) {
                    tiankong.leftImage = NO;
                    make.right.mas_equalTo(lastview.mas_left).with.offset(-LENGTH(59));
                    make.bottom.mas_equalTo(lastview.mas_top).with.offset(-LENGTH(263));
                }else{
                    if (i%2 == 0) {
                        tiankong.leftImage = YES;
                        make.left.mas_equalTo(lastview.mas_right).with.offset(LENGTH(266));
                        make.bottom.mas_equalTo(lastview.mas_top).with.offset(-LENGTH(150+arc4random()%74));
                    }else{
                        tiankong.leftImage = NO;
                        
                        make.right.mas_equalTo(lastview.mas_left).with.offset(-LENGTH(266));
                        make.bottom.mas_equalTo(lastview.mas_top).with.offset(-LENGTH(150+arc4random()%236));
                    }
                }
            }];
            ...
            [tiankong mas_makeConstraints:^(MASConstraintMaker *make) {
                if (i%2 == 0) {
                    make.right.mas_equalTo(view.mas_left).with.offset(-LENGTH(20));
                    make.bottom.mas_equalTo(view.mas_top).with.offset(LENGTH(60));
                }else{
                    make.left.mas_equalTo(view.mas_right).with.offset(LENGTH(20));
                    make.bottom.mas_equalTo(view.mas_top).with.offset(LENGTH(60));
                }
            }];
        }
        [view mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.height.mas_equalTo(LENGTH(123));
        }];
        if (i == viewarray.count-1) {
            [view mas_makeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo(ws.mas_top).with.offset(LENGTH(114));
            }];
        }
        lastview = view;
...
- (void)addnewBody{
    ...
    for (int i = 0; i < viewarray.count; i++) {
        ...
        if (i == 0) {
            ...
            CGPoint point2 ;
            NSInteger sj = LENGTH(50+arc4random() % 30);
            if (one == 0) {
...
//            FLAnimatedImageView * xz = [[FLAnimatedImageView alloc] initWithFrame:CGRectMake(point2.x, point2.y, LENGTH(62), LENGTH(54))];
...
//            }];
            ...
            if (one == 0 && oneone == 0) {
                point3 = CGPointMake(point.x+(point1.x-point.x)/4+LENGTH(suiji),point.y+(point1.y-point.y)/4*3);
            ...
            if (two == 0 && twotwo == 0) {
                point4 = CGPointMake(point.x+(point1.x-point.x)/4*3+LENGTH(suiji),point.y+(point1.y-point.y)/4);
...
//        border.lineCap = @"square";
        ...
        float fl = self.frame.size.height/leftImage.size.height*1.0;
        int a;
        a = ceil(fl);
        ...
        float fll = self.frame.size.height/rightImage.size.height*1.0;
        int d;
        d = ceil(fll);
        for (int b = 0; b<d; b++) {
