...
//    [viewarray addObject:backView];
    ...
    leftImage.image = UIIMAGE(@"icon_班级动态");
...
//    [backView addSubview:title];
    ...
    [leftImage mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self->backView.mas_left).with.offset(LENGTH(25));
        make.centerY.mas_equalTo(self->backView.mas_centerY);
        make.height.mas_offset(LENGTH(self->leftImage.image.size.height));
        make.width.mas_offset(LENGTH(self->leftImage.image.size.width));
    }];
