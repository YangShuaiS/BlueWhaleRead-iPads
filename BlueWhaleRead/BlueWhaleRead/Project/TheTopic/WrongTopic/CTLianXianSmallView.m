...
- (void)addtiMu{
     WS(ws);
    bookName = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(255, 255, 255) LabelFont:TextFont(25) TextAlignment:NSTextAlignmentLeft Text:@""];
    [self addSubview:bookName];
    [bookName mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(ws);
        make.top.equalTo(ws).with.offset(StatusBar+44+LENGTH(46));
    }];
    
    tiMu = [DTTiMuView new];
    [self addSubview:tiMu];
    [tiMu mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self->bookName.mas_bottom).with.offset(LENGTH(22));
...
-(void)setupUI{
    ...
    for (int i = 0; i<tiarrays.count; i++) {
        ...
        [view mas_makeConstraints:^(MASConstraintMaker *make) {
            if (i == 0) {
                make.top.mas_equalTo(self->tiMu.mas_bottom).with.offset(LENGTH(60));
                
            }else{
                make.top.equalTo(lastView.mas_bottom).with.offset(LENGTH(40));
            }
            make.left.equalTo(ws).with.offset(LENGTH(52));
    ...
    for (int i = 0; i<xuanxiangarrays.count; i++) {
        ...
        [view mas_makeConstraints:^(MASConstraintMaker *make) {
            if (i == 0) {
                ...
                make.top.equalTo(lastViews.mas_bottom).with.offset(LENGTH(40));
            }
            make.right.equalTo(ws).with.offset(-LENGTH(52));
            make.width.mas_equalTo(LENGTH(230));
            make.height.mas_equalTo(LENGTH(60));
