...
//    }];
    leixing = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(254,165,79) LabelFont:TextFont(22) TextAlignment:NSTextAlignmentCenter Text:@""];
    leixing.backgroundColor = RGB(255,255,255);
    leixing.layer.masksToBounds = YES;
    leixing.layer.cornerRadius = LENGTH(20);
    [self addSubview:leixing];
    
    [leixing mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(ws).with.offset(LENGTH(38));
    ...
    [title mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(ws).with.offset(LENGTH(34));
        make.right.equalTo(ws).with.offset(-LENGTH(40));
        make.left.equalTo(self->leixing.mas_right).with.offset(LENGTH(16));
        make.bottom.equalTo(ws).with.offset(-LENGTH(34));
    }];
