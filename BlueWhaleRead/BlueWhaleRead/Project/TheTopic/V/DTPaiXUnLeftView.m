...
-(void)setupUI{
    BaseLabel * max = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(148, 129, 42) LabelFont:TextFont(20) TextAlignment:NSTextAlignmentCenter Text:@"第一"];
    max.backgroundColor = RGB(252, 228, 117);
    max.layer.masksToBounds = YES;
    max.layer.cornerRadius = LENGTH(30);
    [self addSubview:max];
    
    BaseLabel * dian = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(252,228,117) LabelFont:TextFont(44) TextAlignment:NSTextAlignmentCenter Text:@"·\n·\n·\n"];
    dian.numberOfLines = 0;
    [self addSubview:dian];
    
    mix = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(148, 129, 42) LabelFont:TextFont(20) TextAlignment:NSTextAlignmentCenter Text:@"最后"];
    mix.backgroundColor = RGB(252, 228, 117);
    mix.layer.masksToBounds = YES;
    mix.layer.cornerRadius = LENGTH(30);
    [self addSubview:mix];
