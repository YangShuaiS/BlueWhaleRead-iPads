...
- (void)addview{
    ...
    backView.layer.cornerRadius = LENGTH(45)/2;
    backView.layer.masksToBounds = YES;
    [downView addSubview:backView];
    [backView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(downView.mas_left).with.offset(LENGTH(136));
        make.right.equalTo(downView.mas_right).with.offset(-LENGTH(136));
...
- (void)setupViewControllerScroll
{
    ...
    [scrollView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(ws).with.insets(UIEdgeInsetsMake(LENGTH(76), 0, 0, 0));
        scrollView.contentSize = CGSizeMake(scrollView.frame.size.width*self->_controllerArray.count, scrollView.frame.size.height-LENGTH(76));
