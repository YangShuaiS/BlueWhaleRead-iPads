...
- (void)addView{
    ...
    WS(ws);
    _zhanghao = [UserForgetTextFileView new];
    _zhanghao.title = @"请输入预留手机号";
    _zhanghao.keyboardType = UIKeyboardTypeNumberPad;
    [self addSubview:_zhanghao];
    [_zhanghao mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(ws);
        make.left.mas_equalTo(ws);
        make.right.mas_equalTo(ws);
        make.height.mas_equalTo(LENGTH(90));
    }];
    [_zhanghao setBlock:^(NSString *text) {
        [ws phone:text];
        [ws next];
    }];
    ...
    [xian mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(ws);
        make.right.mas_equalTo(ws);
        make.top.mas_equalTo(ws.zhanghao.mas_bottom);
...
- (NSArray *)Whetherornotthesame{
    return @[_zhanghao.textField.text,mima.textField.text];
...
- (void)next{
    BOOL phone = [BaseObject valiMobile:_zhanghao.textField.text];
