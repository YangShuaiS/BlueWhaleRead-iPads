...
- (void)viewDidLoad {
    ...
    userImageView.layer.cornerRadius = LENGTH(130)/2;
    userImageView.backgroundColor = RGB(233,248,254);
    [self.view addSubview:userImageView];
    [userImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo(ws.view);
        make.top.mas_equalTo(ws.view.mas_top).with.offset(LENGTH(136));
    ...
    textFile.layer.cornerRadius = LENGTH(20);
    [self.view addSubview:textFile];
    [textFile mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo(ws.view);
        make.top.mas_equalTo(self->userImageView.mas_bottom).with.offset(LENGTH(80));
        make.width.mas_equalTo(LENGTH(492));
    }];
    
    BaseLabel * dl = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(233,249,254) LabelFont:TextFont(22) TextAlignment:NSTextAlignmentCenter Text:@"登录"];
    dl.backgroundColor = RGB(32,210,242);
    dl.layer.masksToBounds = YES;
    dl.layer.cornerRadius = LENGTH(33);
    [self.view addSubview:dl];
    [dl mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo(ws.view);
        make.top.mas_equalTo(self->textFile.mas_bottom).with.offset(LENGTH(156));
        make.width.mas_equalTo(LENGTH(295));
