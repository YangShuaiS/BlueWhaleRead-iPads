//
//  UserLoginForgetMMOneViewController.m
//  BlueWhaleRead
//
...
@interface UserLoginForgetMMOneViewController ()<NavDelegate>

@end

@implementation UserLoginForgetMMOneViewController{
...
- (void)AddNavtion{
    ...
    [textFile mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(ws.navtive.mas_bottom).with.offset(LENGTH(70));
        make.left.mas_equalTo(LENGTH(84));
        make.right.mas_equalTo(-LENGTH(84));
    }];
    ...
    dl = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(255,255,255) LabelFont:TextFont(20) TextAlignment:NSTextAlignmentCenter Text:@"下一步"];
...
- (void)UpData{
    UserLoginForgetMMTwoViewController * vc = [UserLoginForgetMMTwoViewController new];
    vc.phone = textFile.zhanghao.textField.text;
