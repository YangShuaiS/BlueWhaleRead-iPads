...
- (void)viewDidLoad {
    ...
    [title mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(bakView.mas_top).with.offset(LENGTH(30));
        make.centerX.mas_equalTo(bakView);
    }];
    
    subtitle = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:ChangYongTitLe LabelFont:TextFont(20) TextAlignment:NSTextAlignmentLeft Text:@""];
    subtitle.numberOfLines = 0;
    [bakView addSubview:subtitle];
    [subtitle mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self->title.mas_bottom).with.offset(LENGTH(37));
        make.left.mas_equalTo(bakView.mas_left).with.offset(LENGTH(50));
        make.right.mas_equalTo(bakView.mas_right).with.offset(-LENGTH(50));

    }];
    
    time = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(137,159,159) LabelFont:TextFont(18) TextAlignment:NSTextAlignmentRight Text:@"2018-05-07  17:41"];
    [bakView addSubview:time];
    [time mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(bakView.mas_right).with.offset(-LENGTH(52));
        make.top.mas_equalTo(self->subtitle.mas_bottom).with.offset(LENGTH(20));
        make.bottom.mas_equalTo(bakView.mas_bottom).with.offset(-LENGTH(54));
    }];
    
    remove = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(254,127,124) LabelFont:TextFont(20) TextAlignment:NSTextAlignmentCenter Text:@"删除"];
    remove.backgroundColor = [UIColor clearColor];
    remove.layer.masksToBounds = YES;
    remove.layer.cornerRadius = LENGTH(25);
    remove.layer.borderWidth = LENGTH(2);
