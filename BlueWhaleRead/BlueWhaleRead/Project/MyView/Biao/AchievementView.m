...
-(void)setupUI{
    ...
    title = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(18,41,41) LabelFont:TextFont(25) TextAlignment:NSTextAlignmentCenter Text:@""];
    ...
    titleDown = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(50,73,73) LabelFont:TextFont(20) TextAlignment:NSTextAlignmentCenter Text:@""];
    [self addSubview:titleDown];
    

    subtitle = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(50,73,73) LabelFont:TextFont(18) TextAlignment:NSTextAlignmentLeft Text:@""];
...
//        subtitle.text = [NSString stringWithFormat:@"%@同学阅读均衡指标为“均衡”\n目前所属级别为%@级,该学生完成了%@级阅读范围内书籍.其中:\n%@类:%@本\n%@类:%@本\n%@类:%@本\n%@类:%@本\n%@类:%@本",Me.name,_readBalance.myLevel,_readBalance.myLevel,one.catalogName,one.catalogNum,two.catalogName,two.catalogNum,three.catalogName,three.catalogNum,four.catalogName,four.catalogNum,five.catalogName,five.catalogNum];
    ...
    [titleDown mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(radar.mas_bottom).with.offset(LENGTH(26));
        make.left.equalTo(ws).with.offset(LENGTH(50));
        make.right.equalTo(ws).with.offset(-LENGTH(50));
    }];
    
    [subtitle mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self->titleDown.mas_bottom).with.offset(LENGTH(20));
        make.left.equalTo(ws).with.offset(LENGTH(50));
        make.right.equalTo(ws).with.offset(-LENGTH(50));
    }];
    ...
    [xiankuang mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(ws).with.offset(LENGTH(45));
...
- (void)addRadarNengLi{
    ...
    if (_readPower.is_power == 0) {
        ...
        NSString * all =[NSString stringWithFormat:@"不达标项有：\n\n提取信息能力：得分 %@ 分，超过班级 %@%% 的同学\n整体感知能力：得分 %@ 分，超过班级 %@%% 的同学\n形成解释能力：得分 %@ 分，超过班级 %@%% 的同学\n作出评价能力：得分 %@ 分，超过班级 %@%% 的同学\n解决问题能力：得分 %@ 分，超过班级 %@%% 的同学",onefen,onefenbanfenbi,twofen,twofenbanfenbi,threefen,threefenbanfenbi,fourfen,fourfenbanfenbi,fivefen,fivefenbanfenbi];
...
//    item3.itemColor = RGBA(156, 133, 62, 0.5);
    ...
    [his1 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(his.mas_bottom).with.offset(LENGTH(24));
        make.left.equalTo(ws).with.offset(LENGTH(42));
        make.right.equalTo(ws).with.offset(-LENGTH(42));
        make.bottom.mas_equalTo(ws).with.offset(-LENGTH(32));
        make.height.mas_equalTo(LENGTH(350));
...
//    }
    ...
    subtitle.attributedText = AttributedStrsu;
    [subtitle mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(his.mas_bottom).with.offset(LENGTH(20));
        make.left.equalTo(ws).with.offset(LENGTH(50));
        make.right.equalTo(ws).with.offset(-LENGTH(50));
        make.bottom.mas_equalTo(ws).with.offset(-LENGTH(32));
    }];
