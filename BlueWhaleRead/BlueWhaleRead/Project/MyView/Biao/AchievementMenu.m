...
- (void)setModel:(AchievementModel *)model{
    ...
    for (AchievementReportModel * m in model.report) {
        BaseLabel * label = [[BaseLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) LabelTxteColor:RGB(255,255,255) LabelFont:TextFont(20) TextAlignment:NSTextAlignmentCenter Text:m.report_name];
    ...
    CGFloat width = (WIDTH-LENGTH(46)*2)/model.report.count;
    for (int i = 0; i < viewarray.count; i++) {
        ...
        if (i == 0) {
            [label mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                make.left.equalTo(ws).with.offset(LENGTH(46));
            ...
            [label mas_makeConstraints:^(MASConstraintMaker *make) {
                make.centerY.equalTo(self->lastlabel.mas_centerY).with.offset(0);
                make.left.equalTo(self->lastlabel.mas_right).with.offset(0);
                make.right.equalTo(ws).with.offset(-LENGTH(46));
            }];
        }
        [label mas_makeConstraints:^(MASConstraintMaker *make) {
            make.width.mas_equalTo(width);
